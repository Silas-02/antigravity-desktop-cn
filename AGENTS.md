...
- 当前汉化发布版本为 **v2.15.0**，匹配官方 **Antigravity v2.15.0** 客户端。当前版本独立词典为 `dicts/v2.15.0.json`；请始终保持此版本号、发布日期与 `README.md` 中的支持声明一致。
...
- 所有字典与引擎规则必须严格遵循以下标点一致性规范：
  ...
  - **标点符号统一**：中文语境下严禁使用英文句号 `.` 代替中文句号 `。`，且类似句式之间不得混用 `.` 与 `。`。疑问句统一使用全角问号 `？`；键值分隔符使用全角冒号 `：`；加载中或占位符结尾的省略号统一保留半角 ASCII `...`。技术标识符（如版本号 `v2.15.0`、扩展名 `.json`、快捷键 `Ctrl+L`）一律保留半角 ASCII 点号。
