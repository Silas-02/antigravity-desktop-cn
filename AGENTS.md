...
- 当前汉化发布版本为 **v2.17.0**，匹配官方 **Antigravity v2.17.0** 客户端。当前版本独立词典为 `dicts/v2.17.0.json`；请始终保持此版本号、发布日期与 `README.md` 中的支持声明一致。
...
- **严格遵循 1:1 官方源文对应，严禁添加任何冗余变体**：字典键名必须且仅保留与官方客户端源码、解包元数据或实际界面渲染严格一致的官方英文源文，全局保持唯一映射。严禁为了防止匹配遗漏而人为录入两份或多份冗余的变体键名（例如：严禁同时添加“带末尾句号”与“人为去除句号”的双份词条、严禁同时添加“带尾随空格”与“人为去除空格”的双份词条、严禁生成任何猜测性的派生副本）。汉化只需以唯一真实的官方源文作为键名，翻译为符合标点规范的中文即可。
...
- 所有字典与引擎规则必须严格遵循以下标点一致性规范：
  ...
  - **标点符号统一**：中文语境下严禁使用英文句号 `.` 代替中文句号 `。`，且类似句式之间不得混用 `.` 与 `。`。疑问句统一使用全角问号 `？`；键值分隔符使用全角冒号 `：`；加载中或占位符结尾的省略号统一保留半角 ASCII `...`。技术标识符（如版本号 `v2.17.0`、扩展名 `.json`、快捷键 `Ctrl+L`）一律保留半角 ASCII 点号。
  ...
  - **严禁臆测性标点变体冗余**：严禁为了适配有无句号而人为录入双份词条（如同时录入 `"Text"` 与 `"Text."`）。官方源文有句号就录入带句号的键，无句号就录入不带句号的键，译文按零末尾句号规范统一输出中文，严禁在词典中制造重复冗余。
